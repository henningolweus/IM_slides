// html2pptx-fast.js — variant of html2pptx.js that accepts a pre-launched
// browser context, so callers can reuse one Chromium across many slides.
// This cuts ~2s of browser-launch overhead per slide → ~9 min saved over 264.
//
// Public API:
//   await html2pptxFast(htmlPath, pres, { ctx })
//
// The caller is responsible for launching/closing the browser context.
...
export async function html2pptxFast(htmlPath, pres, { ctx }) {
  // Reuse the provided context — open a fresh page per slide.
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      function hasDirectText(el) { return hasFlowingText(el); }

      function ancestorWouldCaptureText(el) {
        let p = el.parentElement;
        while (p) {
          if (TEXT_TAGS.has(p.tagName) || LIST_TAGS.has(p.tagName)) return true;
          p = p.parentElement;
        }
        return false;
      }
      ...
      // positioned ancestor that has one (CSS stacking context).
      ...
      function effectiveTextAlign(el, s) {
        const own = (s.textAlign || '').toLowerCase();
        if (own && own !== 'start' && own !== 'left' && own !== '') return own;
        let cur = el.parentElement;
        while (cur && cur !== document.body) {
          const cs = getComputedStyle(cur);
          const ta = (cs.textAlign || '').toLowerCase();
          if (ta && ta !== 'start' && ta !== 'left' && ta !== '') return ta;
          cur = cur.parentElement;
        }
        return 'left';
      }
      ...
      function extractLiRunsPage(li, inheritedState) {
        ...
        function walk(node, state) {
          ...
          if (node.nodeType === 1) {
            ...
            for (const c of (node.childNodes || [])) walk(c, childState);
          }
        }
      ...
      function flattenNestedULPage(ul, level) {
        ...
        for (const li of (ul.children || [])) {
          ...
          if (directRuns.length > 0) out.push({ level: lvl, runs: directRuns });
          const childUls = Array.from(li.children || []).filter(c => UL_TAGS_PAGE.has(c.tagName));
          for (const childUl of childUls) out.push.apply(out, flattenNestedULPage(childUl, lvl + 1));
        }
        return out;
      }

      function buildTextFrame(el, s, r, domIndex) {
      ...
      function buildShapeText(el, s, r, domIndex, bgChain) {
        // Merges what buildShape and buildTextFrame would have produced for the same element.
        const tf = buildTextFrame(el, s, r, domIndex);
        if (!tf) return null;
        const sh = buildShape(el, s, r, domIndex, bgChain);
        // Copy shape fields onto the text frame, then return as a new kind.
        return {
          ...tf,
          kind: 'shape-text',
          // Shape fields:
      ...
      function buildList(el, s, r, domIndex) {
      ...
      for (const el of all) {
        ...
        const hasShape = hasFill || hasBgImage || hasBorder;
        ...
        const emitsSelfText = (isTextTag && !ancestorOwnsText) || isBareTextDiv;

        // Chevron clip-path can't host text reliably in pptxgenjs — fall back to overlay.
        const chevHint = s.clipPath && s.clipPath !== 'none' && s.clipPath.startsWith('polygon(');
        // Rotated shape-with-text doesn't compose cleanly with addText({shape: rect})
        // margin/fill semantics — fall back to the legacy split path which handles
        // rotation correctly via rotated-bbox math.
        const rotated = detectRotationDeg(el, s) !== 0;

        if (hasShape && emitsSelfText && !chevHint && !rotated) {
          // ONE record — shape with text inside (Fix 3)
          domIndex++;
          const st = buildShapeText(el, s, r, domIndex, bgChain);
          if (st) out.push(st);
        } else {
          // Legacy path — shape and text-frame as separate records
          if (hasShape) {
            domIndex++;
        ...
        if (LIST_TAGS.has(tag)) {
          ...
          if (underUL) continue;
          domIndex++;
          const list = buildList(el, s, r, domIndex);
      ...
      if (slideEl) {
        ...
        if (sBg && sBg !== 'rgba(0, 0, 0, 0)' && sBg !== 'transparent') slideBg = cssColor(sBg);
      }
    ...
    // Node-side post-processing (identical to html2pptx.js)
    // Apply to both 'shape' and 'shape-text' — both carry a fill that needs alpha-blending.
    for (const el of result.elements) {
      if (el.kind !== 'shape' && el.kind !== 'shape-text') continue;
      if (!el.ownBg && el.bgImage && el.bgImage !== 'none' && el.bgImage.includes('gradient(')) {
        const gradHex = parseGradient(el.bgImage);
        if (gradHex) { el.fill = gradHex; continue; }
      }
    ...
    function shapeTextShape(el) {
      // Same as pickShapeType but never returns ellipse — text inside an
      // ellipse renders unreliably in PowerPoint with addText({shape}).
      const radius = el.radius || 0;
      if (radius > 1) return pres.ShapeType.roundRect;
      return pres.ShapeType.rect;
    }
    ...
    if (slideBg) slide.background = { color: slideBg };
    const warnings = [];
    ...
    for (const el of elements) {
      ...
      if (el.kind === 'shape') {
        ...
        if (thinH && !thinW) {
          ...
          slide.addShape(pres.ShapeType.rect, { x, y, w, h: Math.max(0.01, h), fill: { color: lineColor }, line: { type: 'none' } });
          continue;
        }
        if (thinW && !thinH) {
          const b = el.borders || {};
          const lineColor = el.fill || (b.left && b.left.c) || (b.right && b.right.c) || '67817F';
          slide.addShape(pres.ShapeType.rect, { x, y, w: Math.max(0.01, w), h, fill: { color: lineColor }, line: { type: 'none' } });
          continue;
        }
        ...
        if (allFour && sameWidth && sameColor) {
          ...
          for (const side of sides) {
            ...
            if (dash === 'dash' && bw > 0) {
              slide.addShape(pres.ShapeType.line, { x: sx, y: sy, w: sw, h: sh, line: { color: bc, width: bw, dashType: 'dash' } });
            } else {
              slide.addShape(pres.ShapeType.rect, { x: sx, y: sy, w: sw, h: sh, fill: { color: bc }, line: { type: 'none' } });
            }
          }
        }
        continue;
      ...
      if (el.kind === 'shape-text') {
        // Merged emit: a single shape with text inside.
        // CSS padding (on el.padding) becomes pptxgenjs `margin`
        // (in points; px → pt = 0.75).
        ...
        const b = el.borders || { top:{w:0}, right:{w:0}, bottom:{w:0}, left:{w:0} };
        const presentSides = ['top','right','bottom','left'].filter(side => b[side] && b[side].w > 0);
        const allFour = presentSides.length === 4;
        const sameWidth = allFour && new Set(presentSides.map(side => b[side].w)).size === 1;
        const sameColor = allFour && new Set(presentSides.map(side => b[side].c)).size === 1;
        const uniformBorder = allFour && sameWidth && sameColor;
        const dash = parseBorderStyle(el.borderStyle);

        const lineOpts = uniformBorder
          ? { color: b.top.c, width: b.top.w, ...(dash === 'dash' ? { dashType: 'dash' } : {}) }
          : { type: 'none' };

        const opts = {
          x, y, w, h,
          shape: shapeTextShape(el),
        ...
        // Non-uniform border? Emit per-side strips OVER the shape, like the legacy path.
        if (!uniformBorder) {
          for (const side of presentSides) {
            const bw = b[side].w;
            const bc = b[side].c || '67817F';
            const bs = b[side].style;
            const sideDash = parseBorderStyle(bs);
            const bwIn = PX_TO_IN(bw);
            let sx = x, sy = y, sw = w, sh = h;
            if (side === 'top') sh = bwIn;
            if (side === 'bottom') { sy = y + h - bwIn; sh = bwIn; }
      ...
      if (el.kind === 'list') {
        const inner = paddedFrame(el);
        const opts = {
          x: inner.x, y: inner.y, w: inner.w + 0.02, h: inner.h + 0.06,
          fontFace: el.fontFamily,
          fontSize: el.fontSize * 0.75,
          color: el.color || '000000',
          valign: 'top',
      ...
      if (el.kind === 'image') {
        ...
        else opts.path = el.src.replace(/^file:\/\/\//, '');
        slide.addImage(opts);
        continue;
      }
    }
    ...
    for (const { box, dataUri } of svgImages) {
      slide.addImage({
        data: dataUri,
        x: PX_TO_IN(box.x), y: PX_TO_IN(box.y),
        w: PX_TO_IN(box.w), h: PX_TO_IN(box.h),
      });
    }

    return { warnings };
  } finally {
    // CRITICAL: close the page so the context releases its resources.
    await page.close();
  }
}
